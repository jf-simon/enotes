...
//                   char buf[PATH_MAX];
                  ...
                  Eina_Strbuf *last_modified, *put_data_existing, *newbuf, *logindata, *header_url_report, *tmp, *tmp1, *curl_url, *buf, *tmpnew;
...
printf("note_list_put: %i\n", eina_list_count(note_list_put));
   EINA_LIST_FOREACH(note_list_put, l, list_data) //TODO: checken ob href vorhanden, wenn nicht, neue erstellen ansonsten wird es ein update mit gleicher hred
         {
...
//                      "START:20190219T153000Z\n"
               ...
               //       "DTSTAMP:20190205T153000Z\n"
                     ...
                     "UID:%s\n"
                     "END:VTODO\n"
                     "END:VCALENDAR\n", list_data->Note_Sync_Data.prodid, list_data->Note_Sync_Data.created, list_data->Note_Sync_Data.last_modified, list_data->note_name, list_data->Note_Sync_Data.status, list_data->note_text, list_data->Note_Sync_Data.uid);
...
//                   printf("PUTDATA:\n%s\n", eina_strbuf_string_get(put_data_existing));
               ...
               //     curl_easy_setopt (curl, CURLOPT_VERBOSE, "on"); 
                  ...
                  if(strcmp(list_data->Note_Sync_Data.href, "")) // ceate new note online
                  {
                     ...
                     eina_strbuf_append_printf(tmp, "PUT %s HTTP/1.1", list_data->Note_Sync_Data.href);
                     
                  }
                  else
                  {
                     eina_strbuf_append_printf(tmp1, "https://enotes.ocloud.de/remote.php/dav/calendars/enotes/personal/enotes_%s.ics", list_data->Note_Sync_Data.last_modified);
                     eina_strbuf_append_printf(tmp, "PUT /calendars/enotes/personal/enotes_%s HTTP/1.1", list_data->Note_Sync_Data.last_modified);
...
//                   if(list_data->Note_Sync_Data.href == NULL || !strcmp(list_data->Note_Sync_Data.href, "")) // ceate new note online
//                   {
...
//                      list_data->Note_Sync_Data.href = eina_stringshare_add(eina_strbuf_string_get(tmp));
//                      printf("href UPDATE\n");
//                   }
