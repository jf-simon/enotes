...
/*
static void
_save_and_check_user_data(void *data, Evas_Object *obj EINA_UNUSED, void
...
_user_data_changed(void* data, Evas_Object* obj, void* event_info EINA_UNUSED)
{
  ...
  snprintf(
    buf, sizeof(buf), "%s/images/online_notchecked.png", PACKAGE_DATA_DIR);
...
{
  Evas_Object* win;
  Evas_Object *bx, *lb, *en_server, *en_user, *en_password, *tb, *sl_refresh, *en_calendar,
    *bn, *o, *ic, *m_check;
  ...
  if (settings_on != 1) {
    ...
    evas_object_smart_callback_add(
      m_check, "changed", _m_check_callback, list_values_check);
    ...
    evas_object_smart_callback_add(
      en_server, "changed", _user_data_changed, ic);
    evas_object_smart_callback_add(en_user, "changed", _user_data_changed, ic);
    evas_object_smart_callback_add(
      en_password, "changed", _user_data_changed, ic);
    evas_object_smart_callback_add(
      bn, "clicked", _save_and_check_user_data, bx);
    ...
    elm_list_item_append(list, "Show all", NULL, NULL, NULL, list);
    li = elm_list_item_append(list, NULL, NULL, NULL, NULL, list);
    ...
    elm_list_item_append(list, "Enotes", NULL, NULL, NULL, list);
    elm_list_go(list);
